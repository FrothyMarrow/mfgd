...
struct CubicSpline {
  ...
  // Spline construction, Burden & Faires - Numerical Analysis 9th,
  // algorithm 3.4
  void InitializeSpline() {
    ...
    for (int j = n - 1; j >= 0; j--) {
      ...
      m_coeffs[j][1] =
          m_points[j + 1] - m_points[j] - m_coeffs[j][2] - m_coeffs[j][3];
    }
  ...
  vec3 SplineAtTime(float t) {
    ...
    vec3 result = m_coeffs[spline][0] + m_coeffs[spline][1] * x +
                  m_coeffs[spline][2] * xx + m_coeffs[spline][3] * xxx;
    return result;
  }
  ...
  float ArcLengthIntegrandSingleSpline(int spline, float t) {
    ...
    vec3 dv = m_coeffs[spline][1] + 2 * m_coeffs[spline][2] * t +
              3 * m_coeffs[spline][3] * tt;
    float xx = dv.x * dv.x;
    float yy = dv.y * dv.y;
    float zz = dv.z * dv.z;
  ...
  // Composite Simpson's Rule, Burden & Faires - Numerical Analysis 9th,
  // algorithm 4.1
  ...
  float SimpsonsRule(float t0, float t1) {
    ...
    if (first_spline == last_spline)
      return (SimpsonsRuleSingleSpline(last_spline, t1 - last_spline) -
              SimpsonsRuleSingleSpline(first_spline, t0 - first_spline)) *
             multiplier;

    float sum =
        m_lengths[first_spline] - SimpsonsRuleSingleSpline(first_spline, t0);
  ...
  vec3 ConstVelocitySplineAtTime(float t, float speed) {
    ...
    auto L = [this](float t) -> float { return ArcLengthIntegrand(t); };
