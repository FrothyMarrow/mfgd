...
#define VDebugBreak() ::raise(SIGTRAP);
...
// vtb_debug_print - Forwards the text to the debugger's output pane, if it
// exists. The Windows equivalent is OutputDebugString.
...
#define VAssert(x)                                                             \
  do {                                                                         \
    VPRAGMA_WARNING_PUSH                                                       \
    VPRAGMA_WARNING_DISABLE(4127) /* conditional expression is constant */     \
    if (!(x)) /* Still use the value so that the compiler doesn't complain     \
                 about unused values. */                                       \
      VPRAGMA_WARNING_POP {}                                                   \
    VPRAGMA_WARNING_PUSH                                                       \
    VPRAGMA_WARNING_DISABLE(4127) /* conditional expression is constant */     \
...
// VCheck and VAssert - These are the same thing internally, but VAssert means
// program state is definitely wrong and probably unrecoverable (eg algorithm
...
// VUnimplemented - Indicates code that has not been written yet. Generally I
// don't write code that I don't immediately need, and in branches that need to
// exist for completeness but that I currently don't use, I place a
// VUnimplemented(). This saves time so that I don't write things I don't use,
// and it reduces bugs because I don't write code that has no clients and so is
...
// VUntested - Sometimes where I would put a VUnimplemented, instead I will
// write a general outline for what the code should look like, but drop a
// VUntested() in front. It's often easier to write the code while I still have
// the context in mind, but I don't want to invest the time to debug the code.
// If you see this, it means you shouldn't take for granted that the code has
// been debugged.
...
inline int vmin(int a, int b) { return (a < b) ? a : b; }

inline int vmax(int a, int b) { return (a > b) ? a : b; }

inline long vmin(long a, long b) { return (a < b) ? a : b; }

inline long vmax(long a, long b) { return (a > b) ? a : b; }

inline float vmin(float a, float b) { return (a < b) ? a : b; }

inline float vmax(float a, float b) { return (a > b) ? a : b; }

// VInvalid - I often use unsigned data types as handles. The very last
// representable number is reserved as an invalid value for error checking
// purposes. VInvalid is a type- safe way of handling those invalid values.
// Example:
//
...
inline int vtb__vsnprintf(char *outBuf, size_t size, const char *format,
                          va_list ap) {
  int count = -1;
...
#elif _MSC_VER
#include <windows.h>
VTBDEF void vtb_debug_print(const char *text) { OutputDebugStringA(text); }
#else
#include <stdio.h>
VTBDEF void vtb_debug_print(const char *text) { puts(text); }
#endif
