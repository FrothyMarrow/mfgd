...
inline float lengthsqr(vec3 a) { return dot(a, a); }

inline vec3 normalize(vec3 a) { return a / sqrt(dot(a, a)); }
...
const vec2 g_seaweed_positions[] = {
    vec2(-25, 5),        vec2(-25.5f, 5.5f),  vec2(-25.3f, 5.3f),
    vec2(-24.8f, 5.4f),  vec2(-20, -6),       vec2(-21, -5.7f),
    vec2(-20.5f, -6.1f), vec2(-20.3f, -6.7f), vec2(-19, -4),
    vec2(-18, -3),       vec2(-15, 0.4f),     vec2(-15.5f, 0.9f),
    vec2(-15.3f, 0.6f),  vec2(-14.8f, 0.8f),  vec2(-13, 7),
    vec2(-10, 4),        vec2(8, 5),          vec2(16, -3),
    vec2(13, -1),        vec2(12, 2.5f),      vec2(17, 0),
    vec2(16, 1.5f),      vec2(13, 1),         vec2(14.5f, -3.2f),
    vec2(16.5f, -1.1f),  vec2(17.5f, 2.3f),   vec2(14.3f, 0.2f),
    vec2(13.7f, 1.3f),   vec2(16.8f, 1.1f),
};

const vec2 g_wave_vectors[4] = {
    vec2(0.955f, 0.296f) * 0.1f, vec2(0.866f, 0.5f) * 3.0f,
    vec2(0.866f, -0.5f) * 3.0f, vec2(-1.0f, 0.0f) * 3.0f};

vec2 g_wave_vectors_cube[4] = {vec2(0, 0), vec2(0, 0), vec2(0, 0), vec2(0, 0)};

const float g_wave_magnitudes[4] = {43.0f, 0.014f, 0.02f, 0.025f};

const float g_wave_frequencies[4] = {0.5f, 4.0f, 5.0f, 3.0f};
...
vec2 seaweed_cycloid_ddv(vec2 position, float time, float depth) {
  time = -time;
  return depth *
         (g_wave_vectors_cube[0] * sin(dot(g_wave_vectors[0], position) +
                                       g_wave_frequencies[0] * time) +
          g_wave_vectors_cube[1] * sin(dot(g_wave_vectors[1], position) +
                                       g_wave_frequencies[1] * time) +
          g_wave_vectors_cube[2] * sin(dot(g_wave_vectors[2], position) +
                                       g_wave_frequencies[2] * time) +
          g_wave_vectors_cube[3] * sin(dot(g_wave_vectors[3], position) +
                                       g_wave_frequencies[3] * time));
}
...
void SimulateSeaweed() {
  ...
  if (!g_wave_vectors_cube[0].x) {
    g_wave_vectors_cube[0] = g_wave_vectors[0] * g_wave_vectors[0] *
                             g_wave_vectors[0] * g_wave_magnitudes[0];
    g_wave_vectors_cube[1] = g_wave_vectors[1] * g_wave_vectors[1] *
                             g_wave_vectors[1] * g_wave_magnitudes[1];
    g_wave_vectors_cube[2] = g_wave_vectors[2] * g_wave_vectors[2] *
                             g_wave_vectors[2] * g_wave_magnitudes[2];
    g_wave_vectors_cube[3] = g_wave_vectors[3] * g_wave_vectors[3] *
                             g_wave_vectors[3] * g_wave_magnitudes[3];
  }
  ...
  float player_distance_squared =
      g_seaweed_player_distance * g_seaweed_player_distance;
  ...
  for (int k = 0; k < sizeof(g_seaweed) / sizeof(g_seaweed[0]); k++) {
    for (int n = 1; n < SEAWEED_LINKS; n++) {
      ...
      vec3 lower_link_position =
          g_seaweed[k]
              .m_positions[last_list][n - 1]; // Use last_list because the
                                              // updated value went there.
      ...
      vec2 surge =
          10 * seaweed_cycloid_ddv(vec2(curr_position.x, curr_position.y),
                                   (float)Game()->GetTime(), depth);
      vec3 bouyancy = vec3(0, (float)n, 0);
      vec3 avoid_player = vec3(0, 0, 0);

      vec3 away_from_player =
          curr_position - Game()->m_hPlayer->GetGlobalAABB().GetCenter();
      ...
      // This is just a regular euler integration. Not using previously
      // calculated velocity here to maintain precision.
      vec3 new_position =
          2.0f * curr_position - last_position + acceleration * dt_squared;
      ...
      if (normalized_position !=
          normalized_position) // || length(section) < g_seaweed_link_length /
                               // 2)
        VDebugBreak();
#endif

      g_seaweed[k].m_positions[last_list][n] =
          normalized_position; // last_list will soon become the current, so
                               // write the new value into there.
    }
  }
...
void RenderSeaweed() {
  ...
  mat4 scale;
  scale.SetScale(
      vec3(g_seaweed_thickness, g_seaweed_thickness, g_seaweed_thickness) *
      1.5f);

  for (int k = 0; k < sizeof(g_seaweed) / sizeof(g_seaweed[0]); k++) {
    vec3 centroid =
        (g_seaweed[k].m_positions[g_seaweed_current_list][0] +
         g_seaweed[k].m_positions[g_seaweed_current_list][SEAWEED_LINKS - 1]) *
        0.5f;
    float distance_to_camera =
        length(centroid - Game()->m_hPlayer->GetGlobalOrigin());
    // if (distance_to_camera > 3 && dot((centroid -
    // Game()->m_hPlayer->GetGlobalOrigin()) / distance_to_camera, camera) <
    // 0.5f) 	continue;
    ...
    for (int n = 0; n < SEAWEED_LINKS; n++) {
      ...
      vec3 offset_vector =
          vec3(cos(offset), sin(offset), 0.0) * g_seaweed_thickness;

      Matrix4x4 m;
      m.SetTranslation(g_seaweed[k].m_positions[g_seaweed_current_list][n] +
                       offset_vector);
      c.LoadTransform(m);
  ...
  for (int k = 0; k < sizeof(g_seaweed) / sizeof(g_seaweed[0]); k++) {
    ...
    for (int n = 0; n < SEAWEED_LINKS - 1; n++) {
      // c.LoadTransform(m2);
      //			c.RenderBox(Vector(-1, -1, -1), Vector(1, 1,
      //1));
      c.BeginRenderLines();
